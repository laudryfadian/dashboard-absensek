...
import formCompany from "./form_company/formCompany.vue"
import formUserCompany from "./form_company/formUserCompany.vue";
export default {
  components: { formCompany, formUserCompany },
  data: () => ({
    search: "",
    textSnackbar: "",
    snackbar: false,
  ...
  methods: {
    async fetch() {
      const response = await axios.get("http://localhost:5000/companys");
      const data = response.data;

      this.table.items = data.data.reverse();
    },
    async editItem(id) {
      const { data } = await axios.get("http://localhost:5000/companys/" + id)
      this.modal.id = data.data.id;
      this.modal.name = data.data.name;
      this.modal.address = data.data.address;
